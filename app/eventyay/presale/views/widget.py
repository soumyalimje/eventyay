...
def widget_css(request, **kwargs):
    ...
    if o.settings.presale_widget_css_file:
        try:
            resp = FileResponse(
                default_storage.open(o.settings.presale_widget_css_file),
...
class WidgetAPIProductList(EventListMixin, View):
        ...
        if request.event.has_subevents:
            if 'subevent' in kwargs:
                ...
                if not self.subevent:
                    return self.response({'error': gettext('The selected date does not exist in this event series.')})
            else:
                return self._get_event_list(request, **kwargs)
        else:
            if 'subevent' in kwargs:
                return self.response({'error': gettext('This is not an event series.')})
    ...
    def _get_availability(self, ev, event):
        ...
        if ev.presale_is_running and event.settings.event_list_availability:
            ...
            elif event.settings.waiting_list_enabled and (
                ev.best_availability_state is not None and ev.best_availability_state >= 0
            ):
            ...
            availability['text'] = gettext('from %(start_date)s') % {
                'start_date': date_format(ev.presale_start, 'SHORT_DATE_FORMAT')
            }
    ...
    def _get_date_range(self, ev, event, tz=None):
        ...
        if event.settings.show_times:
            ...
            if (
                event.settings.show_date_to
                and ev.date_to
                and ev.date_from.astimezone(tz).date() == ev.date_to.astimezone(tz).date()
            ):
    ...
    def _serialize_events(self, ebd):
        ...
        for e in ebd:
            ...
            if (
                ...
                and ev.date_to
                and ev.date_from.astimezone(tz).date() == ev.date_to.astimezone(tz).date()
                and event.settings.show_date_to
            ):
    ...
    def _get_event_list(self, request, **kwargs):
        ...
        if hasattr(self.request, 'event') and data['list_type'] not in (
            'calendar',
            'week',
        ):
    ...
    def _get_event_view(self, request, **kwargs):
        ...
        if (
            'cart_id' in request.GET
            and CartPosition.objects.filter(event=request.event, cart_id=request.GET.get('cart_id')).exists()
        ):
