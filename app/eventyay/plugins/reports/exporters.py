...
from eventyay.base.exporter import BaseExporter, MultiSheetListExporter
...
class OverviewReport(Report):
    ...
    def _table_story(self, doc, form_data, net=False):
        ...
        story = [
            Paragraph(
                _('Orders by product') + ' ' + (_('(excl. taxes)') if net else _('(incl. taxes)')),
                headlinestyle,
            ),
            Spacer(1, 5 * mm),
        ]
            ...
            story += [
                Paragraph(
                    _('{axis} between {start} and {end}').format(
                        axis=dict(OverviewFilterForm(event=self.event).fields['date_axis'].choices)[
                            form_data.get('date_axis')
                        ],
                        start=date_format(form_data.get('date_from'), 'SHORT_DATE_FORMAT')
                        if form_data.get('date_from')
                        else '–',
                        end=date_format(form_data.get('date_until'), 'SHORT_DATE_FORMAT')
...
class OrderTaxListReportPDF(Report):
    ...
    def get_story(self, doc, form_data):
        ...
        tax_rates = set(
            a
            for a in OrderFee.objects.filter(order__event=self.event)
        ...
        story = [
            Paragraph(
                _('Orders by tax rate ({currency})').format(currency=self.event.currency),
                headlinestyle,
            ),
            Spacer(1, 5 * mm),
        ]
        ...
        tdata = [
            [
                localize(round_decimal(c, self.event.currency)) if isinstance(c, (Decimal, int, float)) else c
                for c in row
            ]
            for row in tdata
        ]
...
class OrderTaxListReport(MultiSheetListExporter):
                    ...
                    (
                        ...
                        forms.ChoiceField(
                            ...
                            choices=(
                                ...
                                (
                                    'last_payment_date',
                                    _('Date of last successful payment'),
                                ),
                            ),
                            required=False,
                        ),
    ...
    def iterate_orders(self, form_data):
        ...
        headers = [
            ...
            ([str(t) + ' % ' + _('Gross'), str(t) + ' % ' + _('Tax')] for t in tax_rates),
            [],
        )
        yield headers
