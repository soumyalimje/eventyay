...
from eventyay.base.services.talkimport import import_speaker_records
...
class SpeakerImportRecordSerializer(serializers.Serializer):
    ...
    linked_submissions = serializers.ListField(
    ...
    def validate_speaker_extras(self, value):
        if value is not None and not isinstance(value, dict):
            raise serializers.ValidationError('speaker_extras must be an object.')
        return value


class SpeakerImportSerializer(serializers.Serializer):
    speakers = SpeakerImportRecordSerializer(many=True, required=True)
...
class SpeakerImportView(APIView):
    ...
    def post(self, request, organizer, event):
        serializer = SpeakerImportSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        acting_user = request.user if request.user.is_authenticated else None
        with language(request.LANGUAGE_CODE, request.event.settings.region):
            result = import_speaker_records(
                request.event,
                serializer.validated_data['speakers'],
                acting_user,
            )
...
class SpeakerViewSet(
    ...
    def get_serializer(self, *args, **kwargs):
        if self.api_version == LEGACY:  # pragma: no cover
            kwargs['questions'] = (self.request.query_params.get('questions') or '').split(',')
    ...
    def submissions_for_user(self):
        return submissions_for_user(self.event, self.request.user).select_related('event')
