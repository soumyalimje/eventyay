...
class PdfDataSerializer(serializers.Field):
    def to_representation(self, instance: OrderPosition):
        ...
        with language(instance.order.locale, instance.order.event.settings.region):
            ...
            if not hasattr(instance.product, '_cached_meta_data'):
                instance.product._cached_meta_data = instance.product.meta_data
            for k, v in instance.product._cached_meta_data.items():
                res['productmeta:' + k] = v
...
class PriceCalcSerializer(serializers.Serializer):
    product = serializers.PrimaryKeyRelatedField(queryset=Product.objects.none(), required=False, allow_null=True)
    variation = serializers.PrimaryKeyRelatedField(
        queryset=ProductVariation.objects.none(), required=False, allow_null=True
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.fields['product'].queryset = event.products.all()
        self.fields['tax_rule'].queryset = event.tax_rules.all()
        self.fields['variation'].queryset = ProductVariation.objects.filter(product__event=event)
        ...
        if data.get('product'):
            if data.get('product').has_variations:
                if not data.get('variation'):
                    raise ValidationError({'variation': ['You should specify a variation for this product.']})
                else:
                    if data.get('variation').product != data.get('product'):
                        raise ValidationError(
                            {'variation': ['The specified variation does not belong to the specified product.']}
                        )
            elif data.get('variation'):
                raise ValidationError({'variation': ['You cannot specify a variation for this product.']})
        if data.get('attendee_name') and data.get('attendee_name_parts'):
            raise ValidationError(
                {'attendee_name': ['Do not specify attendee_name if you specified attendee_name_parts.']}
            )
...
class WrappedModel:
    ...
    def __getattr__(self, product):
        return getattr(self._wrapped, product)
            ...
            if consume_carts:
                for cp in CartPosition.objects.filter(
                    ...
                    quotas = (
                        ...
                        if cp.variation
                        else cp.product.quotas.filter(subevent=cp.subevent)
                    )
                    for quota in quotas:
            ...
            if not force:
                for i, pos_data in enumerate(positions_data):
                    ...
                    if pos_data.get('subevent'):
                        if (
                            pos_data.get('product').pk in pos_data['subevent'].product_overrides
                            and pos_data['subevent'].product_overrides[pos_data['product'].pk].disabled
                        ):
                            errs[i]['product'] = [
                    ...
                    new_quotas = (
                        ...
                        else pos_data.get('product').quotas.filter(subevent=pos_data.get('subevent'))
                    )
                    if len(new_quotas) == 0:
                        errs[i]['product'] = [
