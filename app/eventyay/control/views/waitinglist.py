...
from eventyay.base.models import Product, WaitingListEntry
...
class WaitingListView(EventPermissionRequiredMixin, PaginationMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'move_end' in request.POST:
            try:
                ...
                messages.success(
                    request,
                    _('The waiting list entry has been moved to the end of the list.'),
                )
                return self._redirect_back()
            except WaitingListEntry.DoesNotExist:
                messages.error(request, _('Waiting list entry not found.'))
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['products'] = Product.objects.filter(event=self.request.event)
        ctx['filtered'] = 'status' in self.request.GET or 'product' in self.request.GET

        productvar_cache = {}
        quota_cache = {}
        any_avail = False
        for wle in ctx[self.context_object_name]:
            if (wle.product, wle.variation, wle.subevent) in productvar_cache:
                wle.availability = productvar_cache.get((wle.product, wle.variation, wle.subevent))
            else:
                ev = wle.subevent or self.request.event
                disabled = (
                    not ev.presale_is_running
                    or (wle.subevent and not wle.subevent.active)
                    or not wle.product.is_available()
                )
                if disabled:
                    ...
                    wle.availability = (
                        ...
                        if wle.variation
                        else wle.product.check_quotas(
                            count_waitinglist=False,
                            subevent=wle.subevent,
                            _cache=quota_cache,
                        )
                    )
                productvar_cache[(wle.product, wle.variation, wle.subevent)] = wle.availability
        ...
        ctx['running'] = self.request.event.live and (
    ...
    def get_sales_estimate(self):
        qs = WaitingListEntry.objects.filter(event=self.request.event, voucher__isnull=True).aggregate(
            s=Sum(Coalesce('variation__default_price', 'product__default_price'))
        )
        return qs['s']
