...
def draw_dt_line(
    ...
    if start or end:
        line_parts.append(get_separator(bool(end), bool(start), False, False) + LR * col_width)
    elif run:
    ...
    for loc1, loc2 in zip(rooms[:-1], rooms[1:]):
        ...
        line_parts += get_line_parts(start1, start2, end1, end2, run1, run2, fill_char=fill_char)
        if run2:
            try:
...
def draw_grid_for_day(day, col_width=20):
    ...
    for hour in rrule.rrule(
        ...
        ending_events = {room: next((e for e in talks_by_room[room] if e.local_end == hour), None) for room in rooms}
        lines.append(
            draw_dt_line(
                hour,
                hour in tick_times,
