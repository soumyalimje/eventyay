...
class SubmissionsListView(LoggedInEventPageMixin, ListView):
    ...
    def information(self):
        return [info for info in self.request.event.information.all() if can_view_information(self.request.user, info)]
...
class SubmissionConfirmView(LoggedInEventPageMixin, SubmissionViewMixin, FormView):
    ...
    def get_object(self):
        return get_object_or_404(self.request.event.submissions, code__iexact=self.kwargs.get('code'))
    ...
    def get_form(self):
        ...
        if not self.request.event.cfp.request_availabilities:
            ...
            form.fields['availabilities'].required = self.request.event.cfp.require_availabilities
        return form
    ...
    def save_formset(self, obj):
        ...
        for form in self.formset.initial_forms:
            if form in self.formset.deleted_forms:
                ...
                obj.log_action('eventyay.submission.resource.update', person=self.request.user)
