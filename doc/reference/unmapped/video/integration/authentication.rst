...
Authentication to eventyay is done exclusively through `JSON Web Tokens`_ (JWT). Unless the eventyay world allows
...
When the user first joins, this token needs to be passed by URL like this::
    ...
    https://demo.Eventyay.events/#token=ey...
...
``iss``
    The issuer identifier for the token, needs to exactly match the configuration of the eventyay world. The string
    is arbitrary, but defaults to ``"any"`` in eventyay' default configuration.

``aud``
    The audience identifier for the token, needs to exactly match the configuration of the eventyay world. The string
    is arbitrary, but defaults to ``"eventyay"`` in eventyay' default configuration.
...
``uid``
    ...
    a new eventyay user is created every time the person joins). If you do not have user IDs in your system, you could
...
``traits``
    ...
    to map the user to a set of permissions inside eventyay. For example, the ticketing system eventyay assignes traits
    of the form ``eventyay-event-1234`` with the ID of the event, ``eventyay-item-5678`` with the ID of the product that has
...
``profile``
    ...
    of eventyay field IDs to values::

        {
            "display_name": "John Doe",
            "fields": {
                "e5b06da1-ca18-4204-8b68-769ff86220a9": "@Eventyay"
            }
        }
