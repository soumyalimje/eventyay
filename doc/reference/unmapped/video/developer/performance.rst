...
The eventyay source tree includes a small load testing tool that opens up many websocket connections to a eventyay
...
setup. To run a more production-like setting, you can run the following commands::
    ...
    $ docker-compose run -p 8375:8375 \
        --entrypoint "gunicorn -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:8375 -w 12 eventyay.asgi:application" \
        server
...
Then, apply your load, e.g. run the load testing tool from above or use eventyay manually. Once you hit Ctrl+C, the
