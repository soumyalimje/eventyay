...
authenticates to eventyay through a ticketing system, they might have a trait for every product category they paid for.

Both the world as well as any room can define *implicit grants* based on those traits. For example if anyone with
**both** the ``eventyay-product-1234`` and the ``eventyay-product-5678`` should get the role ``participant`` in a room,
the configuration would look like this::

    "trait_grants": {
      "participant": ["eventyay-product-1234", "eventyay-product-5678"]
    }

In the configuration frontend, this would be shown as::

    eventyay-product-1234, eventyay-product-5678

It's also possible to have "OR"-type grants::

    "trait_grants": {
      "participant": ["eventyay-event-foo", ["eventyay-product-1234", "eventyay-product-5678"]]
    }

In the configuration frontend, this would be shown as::

    eventyay-event-foo, eventyay-product-1234|eventyay-product-5678
