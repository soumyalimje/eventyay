...
One of eventyay's major selling points is its multi-language capability. We make heavy use of Django's
...
For this purpose, we use ``django-i18nfield`` which started out as part of eventyay and then got refactored into
...
For backwards-compatibility with older parts of eventyay' code base and older plugins, ``eventyay.base.forms`` still
...
Last, but definitely not least, we have the ``language`` context manager (``eventyay.base.i18n.language``) that allows
