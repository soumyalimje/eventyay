...
migrations for them, by running ``python -m eventyay makemigrations``. Your
migrations will be applied when running ``python -m eventyay migrate`` just like
any other migration.
...
this::
    ...
    python -m eventyay makemigrations eventyay_facebook
