...
schedule as "accepted", usually in bulk from the review dashboard (see more in the
:ref:`Review Guide <user-guide-review>` section). This triggers a notification to the
...
Eventyay makes sure to not change the schedule without the organiser’s input:
...
You can learn more about the review process in the Review Guide section below.
...
The scheduling guide helps you manage when and where sessions take place. Sessions can be scheduled once they are in the "accepted" state. Schedule-only items like breaks and social events can be added directly to the schedule without creating full session records.
...
The review process allows designated reviewers to evaluate session proposals using structured criteria. Reviewers can be assigned globally or per-track, and can provide scores and feedback that help inform acceptance decisions.
...
Email notifications are an important part of session management. When sessions change state (e.g., from pending to accepted), automated emails can be sent to speakers. These emails are typically queued in the outbox for review before sending, allowing organisers to customize messages as needed.

.. _`user-guide-sessions`:
