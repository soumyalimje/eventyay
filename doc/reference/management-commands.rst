...
All commands are run with the ``python -m eventyay`` prefix::
...
level of protection if you have to use the shell to write to the database!)::
    ...
    $ python -m eventyay shell --event myevent

If you are absolutely sure that you want to access all events, disable the
scoping protection::

    $ python -m eventyay shell --unsafe-disable-scopes
...
Eventyay detects that you don’t have a ``node_modules`` directory, but it’s your
...
Eventyay development (:ref:`developer-translations`).
